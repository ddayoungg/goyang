...
<script type="text/javascript">
	...
	$(function () {
		...
		UserManagerVO search = new UserManagerVO();
		
		String listSearch = request.getParameter("listSearch");
		if(listSearch == null){
			listSearch = "선택";
		}
		search.setListSearch(listSearch);
		...
		List<UserManagerVO> userList = umDAO.selectSearchUser(search);
		
		%>
		
		<%
		String totalUser = "";
		if(!listSearch.equals("선택") || !textSearch.equals("")){
			totalUser = "검색된 회원수";
		}
		...
		UserManagerVO umVO = umDAO.selectUser(id);
	...
	function showPopup(hasFilter, id) {
		...
		/* console.log($("tr:nth-child(2) td:nth-child(2)"));
		console.log($("tr:nth-child(2) td:nth-child(2)")[0]);
		console.log($("tr:nth-child(2) td:nth-child(2)")[0].id);

		console.log($("jsp:param")[0]); */
	}
	...
	<div class="container">
		<div style="display: flex; justify-content: center; margin: 20px;">
			...
			flex-direction: column; background-color: #f0f6f9; width: 350px; height: 150px;">
				<div style="padding-top: 25px; padding-bottom: 15px"><input type="hidden" id="totalUser" name="totalUser">총 회원 수</div>
				<div><span style="font-size: 40px; font-weight: bold;"><%= userList.size() %></span><span style="padding-left: 5px">명</span></div>
			</div>
		</div>
		...
		<div>
			...
			<table id="member" class="member" style="width: 100%">
				...
				<%for(int i=0; i<userList.size(); i++){ 
				  UserManagerVO userVO = userList.get(i);%>
				<tr id="<%= i %>">
					<td><%= userVO.getName() %></td>
					<td><%= userVO.getId() %></td>
					<td><%= userVO.getJoinDate() %></td>
					<td><input id="userId" name="<%= userVO.getId() %>" type="button" value="상세보기" onclick="showPopup(true,'popup')" class="mainBtn button222"></td>
				</tr>
				<%} %>
			</table>
		</div>
			...
			<div style="width: 800px;">
				...
				<div style="width: 800px">
					<table class="member" style="width: 100%">
						...
						<tr>
							<th>아이디</th>
							<td><input type="hidden" id="id" name="id" value="<%= umVO.getId() %>"><%= umVO.getId() %></td>
						</tr>
						<tr>
							<th>가입일자</th>
						...
						<tr>
							<th>관심코스</th>
							<td><%= umVO.getTourName() %></td>
						</tr>
					</table>
				</div>
						
				<div style="display: flex; align-items: center; justify-content: center; margin-top: 10px;">
					<%if(!outFlag.equals("탈퇴")){ %>
					<input id="delBtn" type="button" value="삭제" class="mainBtn" onclick="showPopup(true,'popup2')">
					<%} %>
					<input type="button" value="확인" class="mainBtn" onclick="history.back()">
				</div>
			</div>
		</div>
	</div>
	    ...
	    <div style="width: 412px;">
			...
			<div style="background-color: #f0f6f9;display: flex;flex-direction: column;align-items: center;height: 280px;">
				...
				<div style="display: flex; align-items: center; justify-content: center; padding: 20px;">
					<input type="button" value="확인" class="mainBtn" onclick="update()">
	  ...
	  <div class="content">
		<div style="width: 412px;">
			...
			<div style="background-color: #f0f6f9;">
				...
				<div style="display: flex; align-items: center; justify-content: center; padding-bottom: 10px;">
					<input type="button" value="확인" class="mainBtn" onclick="location.href='manager_member.jsp'">
				</div>
			</div>
		</div>
	  </div>
