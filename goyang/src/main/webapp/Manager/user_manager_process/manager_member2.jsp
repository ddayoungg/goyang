...
<script type="text/javascript">
	...
	$(function () {
			...
			for(int i=0; i<userList.size(); i++){
				...
				if(outFlag==null || flag.equals(outFlag)){
					...
					`<tr bgcolor='<%=bgcolor%>'>
						...
						<td><%= flag %></td>
						<td><input id="<%=i%>" type="button" value="상세보기" class="mainBtn button222"></td>
					</tr>`;
				<%}}%>
		...
		// 상세보기 버튼 클릭 이벤트
		var users = document.querySelectorAll("table tr");
		console.log(users);
		users.forEach(user => user.addEventListener("click", (event)=>showUserDetail(event)));
		
		function showUserDetail(event) {
			var detailHtml = ``;
			<%
			// UserManagerVO userVO = umDAO.selectUser();
			%>
			detailHtml += 
			`<tr>
				<th>이름</th><td>김도희</td>
			...
			<tr>
				<th>이메일</th><td>irrooee@mail.com</td>
			</tr>
			<tr>
				<th>아이디</th><td>doheekim</td>
			</tr>
			<tr>
				<th>가입일자</th><td>2022-09-09</td>
			</tr>
			<tr>
			...
			<tr>
				<th>주소</th><td>서울특별시 강남구 역삼동</td>
			</tr>
			<tr>
				<th>관심코스</th><td>화요나들이</td>
			</tr>`;
	...
	<div class="container">
		<div style="display: flex; justify-content: center; margin: 20px;">
			<div style="display: flex; align-items: center;flex-direction: column; background-color: #f0f6f9; width: 350px; height: 150px;">
				...
				<div><span style="font-size: 40px; font-weight: bold;"><a href="manager_member2.jsp"><%=userList.size()%></a></span><span style="padding-left: 5px">명</span></div>
			</div>
			<div style="display: flex; align-items: center;flex-direction: column; background-color: #f0f6f9; width: 350px; height: 150px; margin-left: 40px;">
				...
				<div><span style="font-size: 40px; font-weight: bold;"><a href="manager_member2.jsp?outFlag=1"><%=userList.size()-outCnt%></a></span><span style="padding-left: 5px">명</span></div>
			</div>
			<div style="display: flex; align-items: center;flex-direction: column; background-color: #f0f6f9; width: 350px; height: 150px; margin-left: 40px;">
				<div style="padding-top: 25px; padding-bottom: 15px">탈퇴 회원 수</div>
				<div><span style="font-size: 40px; font-weight: bold;"><a href="manager_member2.jsp?outFlag=0"><%=outCnt%></a></span><span style="padding-left: 5px">명</span></div>
			</div>
		</div>
